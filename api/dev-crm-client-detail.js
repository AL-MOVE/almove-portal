...
export default async function handler(req,res){
  ...
  try{
    ...
    const cliente={id,...clienteSnap.data(),totalSessoesConfirmadas:sess.filter(s=>s.estado==='Confirmada').length,marcoAtingido:0,streakSemanas:0,proximoMarco:null,numPlanosTreino:new Set(planos.map(p=>p.nomePlano)).size,modoEspecial:null,ultimoCheckin:ck?{...ck}:null};
    const packAtivo=packsSnap.docs.map(d=>d.data()).find(p=>p.mesAno===mesAtual())||null;
    return responder(res,200,{ok:true,detalhe:{cliente,packAtivo,sessoes:sess.filter(s=>s.mesAno===mesAtual()),totalPacksAntigos:0},notas,avaliacoes,atividade:Array.from(atividade.values()).map(item=>{delete item._exercicios;return item}).sort((a,b)=>String(b.data).localeCompare(String(a.data)))});
