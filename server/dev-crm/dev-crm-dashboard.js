...
export default async function handler(req, res) {
  ...
  try {
    ...
    const clientes = clientesSnap.docs.map(d => ({ id: d.id, ...d.data() })); const clientesPorId = new Map(clientes.map(c => [c.id, c])); const mesReal = mesAtual(); const pedido = String(req.query?.mes || ''); const mes = /^\d{4}-(0[1-9]|1[0-2])$/.test(pedido) ? pedido : mesReal;
