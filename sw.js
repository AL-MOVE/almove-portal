...
<body>
  ...
  <div id="tabInicio" class="tab-conteudo oculto">
    ...
    <div class="card">
      <div class="inicio-topo">
        <div>
          ...
          <div class="inicio-sub">Vamos ver o que temos para hoje.</div>
        </div>
      </div>
    ...
    <div class="card">
      <div class="card-title">Métricas de hoje<span class="selo-exemplo">Exemplo</span></div>
      <div class="metricas-grid">
        <div class="metrica-card">
          <div class="metrica-icone">🦶</div>
          <div class="metrica-valor">6.420</div>
          <div class="metrica-meta">/ 10k passos</div>
          <div class="metrica-barra"><div class="metrica-barra-fill" style="width:64%;background:var(--accent);"></div></div>
        </div>
        <div class="metrica-card">
          <div class="metrica-icone">🔥</div>
          <div class="metrica-valor">480</div>
          <div class="metrica-meta">kcal ativas</div>
          <div class="metrica-barra"><div class="metrica-barra-fill" style="width:55%;background:var(--yellow);"></div></div>
        </div>
        <div class="metrica-card">
          <div class="metrica-icone">💧</div>
          <div class="metrica-valor">1.8L</div>
          <div class="metrica-meta">meta 2.5L</div>
          <div class="metrica-barra"><div class="metrica-barra-fill" style="width:72%;background:#38bdf8;"></div></div>
        </div>
      </div>
    </div>

    <div class="card">
      <div class="card-title">Mensagem do treinador<span class="selo-exemplo">Exemplo</span></div>
      <div class="coach-msg">
        <div class="coach-avatar">A</div>
        <div class="coach-texto">
          <div class="coach-nome-linha">
            <span class="coach-nome">Coach André</span>
            <span class="coach-quando">Há 25 min</span>
          </div>
          <div class="coach-frase">"Excelente descanso ontem! Foco nos agachamentos hoje."</div>
        </div>
      </div>
    </div>
  ...
  <div id="tabHoje" class="tab-conteudo">
    ...
    <div class="card fluxo-hoje oculto" id="fluxoHoje">
      ...
      <div class="fluxo-hoje-passos">
        <div class="fluxo-hoje-etapa" id="fluxoHojeEtapa1"><span class="fluxo-hoje-ponto">1</span>Check-in</div>
        <div class="fluxo-hoje-etapa" id="fluxoHojeEtapa2"><span class="fluxo-hoje-ponto">2</span>Teste</div>
  ...
  <div id="tabConquistas" class="tab-conteudo oculto">
    ...
    <div class="card">
      <div class="card-title">Passos de hoje<span class="selo-exemplo">Exemplo</span></div>
      <div class="gauge-wrap">
        <svg class="gauge-svg" viewBox="0 0 160 160">
          <defs>
            <linearGradient id="gaugeGradiente" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stop-color="#5eead4"/>
              <stop offset="100%" stop-color="#0b5cc2"/>
            </linearGradient>
          </defs>
          <circle class="gauge-track" cx="80" cy="80" r="64"/>
          <circle class="gauge-fill" cx="80" cy="80" r="64"
            stroke-dasharray="402.1" stroke-dashoffset="145" id="gaugePassosCirculo"/>
        </svg>
        <div class="gauge-centro">
          <div class="gauge-valor">6.420</div>
          <div class="gauge-meta">de 10.000 passos</div>
          <div class="gauge-pill">64% atingido</div>
        </div>
      </div>

      <div class="hist-semana" id="histSemanaPassos">
        <!-- preenchido por JS com dados de exemplo -->
      </div>
    </div>

    <div class="card">
      <div class="card-title">Os teus marcos</div>
      <div class="marcos-resumo">
        <div class="marco-resumo-card">
          <div class="marco-resumo-valor" id="marcoTotalTreinos">0</div>
          <div class="marco-resumo-label">Treinos<br>concluídos</div>
        </div>
        <div class="marco-resumo-card">
          <div class="marco-resumo-valor" id="marcoStreakAtual">0</div>
          <div class="marco-resumo-label">Semanas<br>seguidas</div>
        </div>
        <div class="marco-resumo-card">
          <div class="marco-resumo-valor" id="marcoMelhorStreak">0</div>
          <div class="marco-resumo-label">Melhor<br>sequência</div>
        </div>
      </div>
    </div>

    <div class="card">
      <div class="card-title">Sequência de treinos</div>
      <div class="marco-lista" id="marcosListaStreak"></div>
    </div>

    <div class="card">
      <div class="card-title">Volume de treinos</div>
      <div class="marco-lista" id="marcosListaVolume"></div>
    </div>
  ...
  <div id="tabPlanos" class="tab-conteudo oculto">
    ...
    <button type="button" class="btn-contacto-pt" id="opcoesBtnContactoPT">
      <span class="btn-contacto-pt-icone">💬</span>
    ...
    <div class="card">
      ...
      <div class="opcoes-lista">
        <div class="opcoes-item">
          <div class="opcoes-item-icone">📄</div>
...
(function () {
  ...
  function renderInicio(dados) {
    if (dados.primeiroNome) {
      inicioSaudacaoEl.textContent = "Olá, " + dados.primeiroNome;
    }
  ...
  // ---------- Conquistas ----------
  // Passos/histograma continuam como exemplo (sem sensor); os marcos usam
  // dados reais de treinos (EXECUCOES) assim que a API responder.
  (function () {
    const HIST_LETRAS = ["S", "T", "Q", "Q", "S", "S", "D"];
    const HIST_EXEMPLO = [4200, 7800, 6100, 9200, 5400, 8700, 6420]; // exemplo, hoje = último
    const histEl = document.getElementById("histSemanaPassos");
    const maxHist = Math.max.apply(null, HIST_EXEMPLO);
    HIST_EXEMPLO.forEach(function (valor, idx) {
      const ehHoje = idx === HIST_EXEMPLO.length - 1;
      const altura = Math.max(6, Math.round((valor / maxHist) * 100));
      const col = document.createElement("div");
      col.className = "hist-col" + (ehHoje ? " hoje" : "");
      col.innerHTML =
        '<div class="hist-barra-fundo"><div class="hist-barra" style="height:' + altura + '%"></div></div>' +
        '<span class="hist-col-letra">' + HIST_LETRAS[idx] + '</span>';
  ...
  function criarMarcoHtml(marco) {
    const estado = marco.concluido ? "conquistado" : (marco.emProgresso ? "" : "bloqueado");
    let lado;
    if (marco.concluido) {
      lado = '<span class="marco-check">✓</span>';
    } else if (marco.emProgresso) {
      lado = "";
    } else {
      lado = '<span class="marco-cadeado">🔒</span>';
    }
    let progresso = "";
    if (!marco.concluido && marco.meta) {
      const pct = Math.min(100, Math.round((marco.atual / marco.meta) * 100));
      progresso =
        '<div class="marco-progresso"><div class="marco-progresso-fill" style="width:' + pct + '%"></div></div>' +
        '<div class="marco-fracao">' + marco.atual + ' / ' + marco.meta + '</div>';
    }
    return (
      '<div class="marco-item ' + estado + '">' +
        '<div class="marco-icone">' + marco.icone + '</div>' +
        '<div class="marco-corpo">' +
          '<div class="marco-titulo">' + marco.titulo + '</div>' +
          '<div class="marco-desc">' + marco.desc + '</div>' +
          progresso +
        '</div>' +
        lado +
      '</div>'
  ...
  function renderConquistas(dados) {
    const totalTreinos = dados.totalTreinosConcluidos || 0;
    const streakAtual = dados.streakSemanas || 0;
    const melhorStreak = Math.max(dados.melhorStreakSemanas || 0, streakAtual);
    ...
    const marcosStreak = [1, 4, 8, 12].map(function (meta) {
      return {
        icone: "🔥",
        titulo: meta === 1 ? "Primeira semana" : meta + " semanas seguidas",
        desc: meta === 1
          ? "Completa pelo menos 1 treino esta semana."
          : "Mantém pelo menos 1 treino por semana durante " + meta + " semanas seguidas.",
        atual: streakAtual,
        meta: meta,
    ...
    const marcosVolume = [10, 25, 50, 100].map(function (meta) {
      return {
        icone: "💪",
        titulo: meta + " treinos concluídos",
        desc: "Regista um total de " + meta + " treinos.",
        atual: totalTreinos,
        meta: meta,
  ...
  chamarApi("getResumoConquistasPortal", {})
    ...
    .catch(function () {
      // Sem ligação — os marcos ficam a zero até haver rede.
    });
